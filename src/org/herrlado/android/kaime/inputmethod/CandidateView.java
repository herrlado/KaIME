...
public class CandidateView extends View {
	...
	private int mScrollX;// TODO FIXME
	 ...
	 * @param attrs
	 */
	public CandidateView(final Context context, final AttributeSet attrs) {
		super(context, attrs);
		mSelectionHighlight = context.getResources().getDrawable(
				R.drawable.list_selector_background_pressed);// TODO FIXME
		...
		mColorNormal = context.getResources()
				.getColor(R.color.candidate_normal);
		mColorRecommended = context.getResources().getColor(
		...
		mDivider = context.getResources().getDrawable(
				R.drawable.keyboard_suggest_strip_divider);
		...
		mScrollX = getScrollX();// TODO FIXME
				...
				new GestureDetector.SimpleOnGestureListener() {
					...
					public void onLongPress(final MotionEvent me) {
						if (mSuggestions.size() > 0) {
							if (me.getX() + mScrollX < mWordWidth[0]
									&& mScrollX < 10) {
								longPressFirstWord();
							}
						}
	 ...
	 * @param listener
	 */
	public void setService(final KaIME listener) {
		mService = listener;
	}
	...
	protected void onDraw(final Canvas canvas) {
		...
		for (int i = 0; i < count; i++) {
			final CharSequence suggestion = mSuggestions.get(i);
			...
			if (mWordWidth[i] != 0) {
				...
				final float textWidth = paint.measureText(suggestion, 0,
						suggestion.length());
			...
			if (canvas != null) {
				canvas.drawText(suggestion, 0, suggestion.length(), x + X_GAP,
						y, paint);
				paint.setColor(mColorOther);
	...
	public void setSuggestions(final List<CharSequence> suggestions,
			final boolean completions, final boolean typedWordValid,
			final boolean haveMinimalSuggestion) {
		clear();
		if (suggestions != null) {
	...
	public void scrollPrev() {
		int i = 0;
		final int count = mSuggestions.size();
		int firstItem = 0; // Actually just before the first item, if at the
		// boundary
		while (i < count) {
	...
	public void scrollNext() {
		...
		final int count = mSuggestions.size();
		final int rightEdge = mScrollX + getWidth();
		while (i < count) {
	...
	private void updateScrollPosition(final int targetX) {
		if (targetX != mScrollX) {
			// TODO: Animate
	 ...
	 * @param x
	 */
	public void takeSuggestionAt(final float x) {
		mTouchX = (int) x;
		// To detect candidate
		onDraw(null);
		if (mSelectedString != null) {
	...
	private void showPreview(final int wordIndex, final String altText) {
		final int oldWordIndex = mCurrentWordIndex;
		...
		if (oldWordIndex != mCurrentWordIndex || altText != null) {
			if (wordIndex == OUT_OF_BOUNDS) {
				...
				final CharSequence word = altText != null ? altText
						: mSuggestions.get(wordIndex);
				mPreviewText.setText(word);
				mPreviewText.measure(MeasureSpec.makeMeasureSpec(0,
						MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(
						0, MeasureSpec.UNSPECIFIED));
				final int wordWidth = (int) (mPaint.measureText(word, 0, word
						.length()) + X_GAP * 2);
				...
				mPopupPreviewX = mWordX[wordIndex]
						- mPreviewText.getPaddingLeft() - mScrollX;
	...
	private void longPressFirstWord() {
		...
		if (mService.addWordToDictionary(word.toString())) {
			showPreview(0, getContext().getResources().getString(
