...
public class BinaryDictionary extends Dictionary {
	 ...
	 */
	public BinaryDictionary(final Context context, final int resId) {
		if (resId != 0) {
			loadDictionary(context, resId);
		}
	}

	// private native int openNative(AssetManager am, String resourcePath,
	...
	private final void loadDictionary(final Context context, final int resId) {
		// AssetManager am = context.getResources().getAssets();
		// String assetName = context.getResources().getString(resId);
		// mN ativeDict = openNative(am, assetName, TYPED_LETTER_MULTIPLIER,
	...
	public void getWords(final WordComposer codes, final WordCallback callback) {
		...
		for (int i = 0; i < codesSize; i++) {
			...
			System.arraycopy(alternatives, 0, mInputCodes,
					i * MAX_ALTERNATIVES, Math.min(alternatives.length,
							MAX_ALTERNATIVES));
		}
		...
		if (ENABLE_MISSED_CHARACTERS && count < 5) {
			for (int skip = 0; skip < codesSize; skip++) {
				final int tempCount = getSuggestionsNative(mNativeDict,
						mInputCodes, codesSize, mOutputChars, mFrequencies,
	...
	public boolean isValidWord(final CharSequence word) {
		...
		final char[] chars = word.toString().toLowerCase().toCharArray();
