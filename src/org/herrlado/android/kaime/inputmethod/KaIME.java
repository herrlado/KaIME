 ...
 */
public class KaIME extends InputMethodService implements
			...
			case MSG_START_TUTORIAL:
				if (mTutorial == null) {
					if (mInputView.isShown()) {
						mTutorial = new Tutorial(KaIME.this, mInputView);
						mTutorial.start();
					} else {
	...
	private void initSuggest(final String locale) {
		mLocale = locale;
	...
	public void onStartInputView(final EditorInfo attribute,
		...
		if (mCandidateView != null) mCandidateView.setSuggestions(null, false,
				false, false);
		loadSettings();
	...
	public void onDisplayCompletions(final CompletionInfo[] completions) {
		...
		if (mCompletionOn) {
			...
			final List<CharSequence> stringList = new ArrayList<CharSequence>();
			for (int i = 0; i < (completions != null ? completions.length : 0); i++) {
	...
	public void updateShiftKeyState(final EditorInfo attr) {
		final InputConnection ic = getCurrentInputConnection();
	...
	public boolean addWordToDictionary(final String word) {
		mUserDictionary.addWord(word, 128);
		return true;
	}

	private boolean isAlphabet(final int code) {
	...
	private void handleCharacter(int primaryCode, final int[] keyCodes) {
	...
	private void handleSeparator(final int primaryCode) {
	...
	private void toggleCapsLock() {
		mCapsLock = !mCapsLock;
		if (mKeyboardSwitcher.isAlphabetMode()) {
			((LatinKeyboard) mInputView.getKeyboard())
	...
	private boolean isPredictionOn() {
		final boolean predictionOn = mPredictionOn;
	...
	private void updateSuggestions() {
		...
		final List<CharSequence> stringList = mSuggest.getSuggestions(
	...
	public void revertLastWord(final boolean deleteChar) {
		final int length = mComposing.length();
		if (!mPredicting && length > 0) {
			...
			final CharSequence toTheLeft = ic.getTextBeforeCursor(
					mCommittedLength, 0);
	...
	void promoteToUserDictionary(final String word, final int frequency) {
	...
	private void launchSettings() {
		...
		intent.setClass(KaIME.this, KaIMESettings.class);
	...
	private void loadSettings() {
		...
		mShowSuggestions = sp.getBoolean(PREF_SHOW_SUGGESTIONS, true)
				& mQuickFixes;
		final boolean autoComplete = sp.getBoolean(PREF_AUTO_COMPLETE,
	...
	private void showOptionsMenu() {
		...
		final CharSequence itemSettings = getString(R.string.english_ime_settings);

		// CharSequence itemInputMethod =
		// getString(com.android.internal.R.string.inputMethod);
		final CharSequence itemInputMethod = null; // TODO //FIXME
		builder.setItems(new CharSequence[] { itemSettings, itemInputMethod },
				new DialogInterface.OnClickListener() {

					public void onClick(final DialogInterface di,
							final int position) {
						di.dismiss();
						switch (position) {
						case POS_SETTINGS:
		...
		final Window window = mOptionsDialog.getWindow();
	...
	private final long[] mCpsIntervals = new long[CPS_BUFFER_SIZE];
	private int mCpsIndex;

	private void measureCps() {
		if (!KaIME.PERF_DEBUG) return;
	...
	class AutoDictionary extends ExpandableDictionary {
		...
		public AutoDictionary(final Context context) {
			super(context);
		}
		...
		@Override
		public void addWord(final String word, final int addFrequency) {
			...
			if (freq > PROMOTION_THRESHOLD) {
				KaIME.this
						.promoteToUserDictionary(word, FREQUENCY_FOR_AUTO_ADD);
			}
		}
	}
}
