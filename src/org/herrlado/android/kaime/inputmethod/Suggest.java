...
public class Suggest implements Dictionary.WordCallback {
	...
	private final ArrayList<CharSequence> mStringPool = new ArrayList<CharSequence>();
	...
	public Suggest(final Context context, final int dictionaryResId) {
		mContext = context;
	...
	public void setCorrectionMode(final int mode) {
	 ...
	 */
	public void setUserDictionary(final Dictionary userDictionary) {
	 ...
	 */
	public void setContactsDictionary(final Dictionary userDictionary) {
		mContactsDictionary = userDictionary;
	}

	public void setAutoDictionary(final Dictionary autoDictionary) {
	...
	private boolean haveSufficientCommonality(final String original,
		...
		for (i = 0; i < minLength; i++) {
			final char origChar = ExpandableDictionary.toLowerCase(original
					.charAt(i));
			if (origChar == ExpandableDictionary.toLowerCase(suggestion
					.charAt(i))) {
				matching++;
				lessMatching++;
			} else if (i + 1 < suggestionLength
					&& origChar == ExpandableDictionary.toLowerCase(suggestion
							.charAt(i + 1))) {
				lessMatching++;
			}
		}
	 ...
	 */
	public List<CharSequence> getSuggestions(final View view,
			final WordComposer wordComposer,
			final boolean includeTypedWordIfValid) {
		...
		while (i < mSuggestions.size() && i < max) {
			final String suggestedWord = mSuggestions.get(i).toString()
					.toLowerCase();
			final CharSequence autoText = AutoText.get(suggestedWord, 0,
	...
	private void removeDupes() {
		...
		while (i < suggestions.size()) {
			...
			for (int j = 0; j < i; j++) {
				final CharSequence previous = suggestions.get(j);
	...
	private void removeFromSuggestions(final int index) {
		final CharSequence garbage = mSuggestions.remove(index);
	...
	public boolean addWord(final char[] word, final int offset,
		...
		final StringBuilder sb = poolSize > 0 ? (StringBuilder) mStringPool
				.remove(poolSize - 1) : new StringBuilder(32);
		sb.setLength(0);
		sb.append(word, offset, length);
		mSuggestions.add(pos, sb);
		if (mSuggestions.size() > prefMaxSuggestions) {
			final CharSequence garbage = mSuggestions
					.remove(prefMaxSuggestions);
	...
	public boolean isValidWord(final CharSequence word) {
		...
		return (mCorrectionMode == CORRECTION_FULL && mMainDict
				.isValidWord(word))
				|| (mCorrectionMode > CORRECTION_NONE
						&& ((mUserDictionary != null && mUserDictionary
								.isValidWord(word)))
						|| (mAutoDictionary != null && mAutoDictionary
								.isValidWord(word)) || (mContactsDictionary != null && mContactsDictionary
						.isValidWord(word)));
	}
	...
	private void collectGarbage() {
		...
		while (poolSize < mPrefMaxSuggestions && garbageSize > 0) {
			final CharSequence garbage = mSuggestions.get(garbageSize - 1);
			if (garbage != null && garbage instanceof StringBuilder) {
				mStringPool.add(garbage);
				poolSize++;
			}
